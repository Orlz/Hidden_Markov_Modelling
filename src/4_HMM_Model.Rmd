...
This script takes the pre-processed data in the form of the coded variables, reshapes these into one dataframe, and then builds a series of models to test. The following steps are taken: 

Covariates 
1. Determine the number of states by varying the number of states from 1 - X and observing the BIC 
2. Run the model 

```{r}
# quickfix 
test <- data 
data$Organic_Preference[data$Organic_Preference == 0] <- 2
data$Price_Sensitivity[is.na(data$Price_Sensitivity)] <- 2
sum(is.na(data$Price_Sensitivity))
```
...
## Merge the covariate data onto ```data```
...
Parameters: 
First parameter = The response model, determining what is to be modelled. (Dairy, Meat, ReadyMeal and Confectionery responses) 
Second parameter
...
In the below chunk, we will run a global search on a model including all predictors. We'll vary the number of states between 1 - 8 and compare the AIC, BIC, and log-likelihood for each state:
```{r number_of_states}
...
cor.test(data$Age, data$environmental_concern)
```
...
In the below section, we'll build and run models varying the covariates used to calculate the initial and transition probabilities. We'll explore the below X models and use the BIC as criterion to determine whether the predictor improves the model: 
...
#Buying stability 
buying_stability <- data %>% select(1:6)

buying_stability <- buying_stability %>% group_by(id) %>% 
  mutate(hh_Dairy = sum(yDairy), 
         hh_Meat = sum(yMeat), 
         hh_ReadyMeals = sum(yReadyMeal), 
         hh_Confect = sum(yConfectionery))

# Quick fix 
test <- buying_stability %>% distinct(id, .keep_all = T)

#A proportion 
test <- test %>% subset(hh_Dairy > 0) # 940
sum(test$hh_Dairy == 2)

68 * 0.

test <- test %>% subset(hh_Confect > 2)

mean(test$hh_Confect)
590 / 1019
